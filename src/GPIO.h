...
public:
	...
	int readValue(std::string *level); //Read current value of the pin (ARGS *level: Address of string type variable, run command with readValue(&STRING_VAR) and the result will be stored in STRING_VAR) (only works if _direction is 0 (input))
